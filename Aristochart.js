(() => {
  ...
  Aristochart.prototype.getPoints = function(callback) {
    var lines = {}, Xmax = this.x.max, Xmin = this.x.min, Xrange = this.x.range, Ymax = this.y.max, Ymin = this.y.min, Yrange = this.y.range, bx = this.box.x, by = this.box.y, bx1 = this.box.x1, by1 = this.box.y1, Yorigin = by + by1 / Yrange * Ymax, Xorigin = bx;
    for (var key in this.data) {
      if (key == "x")
        continue;
      lines[key] = [];
      ...
      for (var i = 0; i < count; i++) {
        var dx = Xrange / (count - 1) * i, y = currArr[i], rx = Xorigin + bx1 / Xrange * dx, ry = Yorigin - by1 / Yrange * y;
        lines[key].push({ x: dx, y, rx, ry });
        if (callback)
  ...
  Aristochart.point = {
    circle: function(style, rx, ry, x2, y, graph) {
      this.ctx.save();
  ...
  Aristochart.tick = {
    line: function(style, x2, y, type, i) {
      this.ctx.save();
  ...
  Aristochart.axis = {
    line: function(style, x2, y, x1, y1, type) {
      this.ctx.save();
  ...
  Aristochart.label = {
    text: function(style, text, x2, y, type, i) {
      if (i % this.options.label[type].step == 0) {
        var label = style.label[type];
        if (type == "x")
          y = y + (style.tick.major + label.offsetY) * this.resolution;
        if (type == "y")
          x2 = x2 - (style.tick.major + label.offsetX) * this.resolution, y += label.offsetY * this.resolution;
        ...
        this.ctx.fillText(substr[0], x2, y);
      }
    }
  };
  Aristochart.title = {
    text: function(style, text, x2, y, type) {
      this.ctx.save();
      if (type == "x")
        y += style.title.x.offsetY, x2 += style.title.x.offsetX;
      if (type == "y")
        y += style.title.y.offsetY, x2 += style.title.y.offsetX;
